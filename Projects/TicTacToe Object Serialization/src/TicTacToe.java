...
public class TicTacToe {
	...
	public void start() {
		...
		System.out.print("Would you like to load the previous session? (Y/N): ");
		String answer = input.nextLine();
		if (answer.equals("Y")) {
			try {				
				//Let's load our data.
				ObjectInputStream oit = new ObjectInputStream(new FileInputStream("ttt.dat"));
				board = (char[][])oit.readObject();
				currentPlayer = oit.readChar();
				oit.close();
			} catch (Exception e) {
				System.out.println("Error!!!!");
				e.printStackTrace();
			}
		} else {  //init board if anything other than Y is enterred
			initializeBoard();
		}
		do {
			...
			if (win) {
				...
				changePlayer();
				//Here is where we write out to file.
				
					try {		
						ObjectOutputStream output = new ObjectOutputStream(new FileOutputStream("ttt.dat"));
						output.writeObject(board);
						output.writeChar(currentPlayer);
						output.close();
					} catch (Exception e) {
